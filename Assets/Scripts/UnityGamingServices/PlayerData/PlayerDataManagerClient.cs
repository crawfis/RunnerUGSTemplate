...
namespace CrawfisSoftware.PlayerDataManagement
{
    ...
    public class PlayerDataManagerClient : IDisposable
    {
        ...
        private readonly PlayerDataServiceBindings m_PlayerDataBindings;
        ...
        public PlayerDataManagerClient(GameManagerUGS gameManagerUGS, PlayerAuthenticationManager authenticationManager, PlayerDataServiceBindings bindingsProvider, NetworkConnectivityHandler networkHandler)
        {
            m_GameManagerUGS = gameManagerUGS;
            m_AuthenticationManager = authenticationManager;
            m_NetworkHandler = networkHandler;
            m_PlayerDataBindings = bindingsProvider;
        ...
        private async void HandleSignInInitialization(string eventName, object sender, object data)
        {
            if (m_IsInitializing)
            {
            ...
            // TODO: Re-enable initialization with retry logic
            try
            {
        ...
        private async Task<bool> InitializePlayerWithRetry()
        {
            ...
            for (int attempt = 0; attempt < MaxInitializeRetries; attempt++)
            {
                try
                {
                    ...
                    var initResponse = await m_PlayerDataBindings.OnSignInHandlePlayerInitialization();
        ...
        private async void HandleLevelWon()
        {
            try
            {
                m_CloudPlayerData = await m_PlayerDataBindings.HandleLevelWon();
        ...
        private async void HandleReplayLevelLost()
        {
            try
            {
                var playerData = await m_PlayerDataBindings.HandleLevelLost();
        ...
        private async void HandleConnectivityChanged(bool isOnline)
        {
            ...
            try
            {
                ...
                var playerData = await m_PlayerDataBindings.GetPlayerData();
                if (playerData != null)
                {
                    m_CloudPlayerData = playerData;
                    PlayerDataUpdated?.Invoke(m_CloudPlayerData);
