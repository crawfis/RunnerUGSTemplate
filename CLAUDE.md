...
| Utilities | `Runtime/Utility/` — `Logger`, `DebugEventFileLogger`, `TimedEvent`, `TextureExtensions`, `Wait` (`Wait.ForSecondsRealtime`, the unscaled Awaitable wait — see Gotchas, *Async and coroutines*); `Runtime/Events/EventHistory.cs` |
...
- This project uses Unity's `Awaitable`, not coroutines: `async Awaitable Foo()`, started
  ...
  `Input/LeftRightJumpSlide.cs` are `IEnumerable` implementations, not coroutines), and none
  in `com.crawfissoftware.common` from 0.5.0 (`TimedEvent` and
  `LoadSceneAfterGameControlEvent` were converted there).
...
Use it instead of `dotnet build` on the generated csproj — see
[Unity CLI and the Pipeline Package](#unity-cli-and-the-pipeline-package).

### Run the test suite
`unity test --mode EditMode` (or `PlayMode`) runs the project's tests and writes a report.
Useful flags: `--affected` (only tests a change can reach), `--rerun-failed`, `--retries <n>`
to surface flaky tests, `--report-format junit` for CI.
